...
public class SEBConfigFileManager
{
    ...
    public static DictObj DecryptSEBSettings(byte[] sebData, bool forEditing, ref string sebFilePassword, ref bool passwordIsHash, ref X509Certificate2 sebFileCertificateRef, bool suppressFileFormatError = false)
    {
        ...
        if (prefixString.CompareTo(MULTIPART_MODE) == 0)
        {
            ...
            Console.WriteLine("Multipart Config File, first part (settings) length: " + multipartConfigLength);

            try
            {
                Console.WriteLine("Cropping config file, as this SEB version cannot process additional parts of multipart config files.");
            ...
            {
                Console.WriteLine("Error while cropping config file", null, ex, ex.Message);
            }
        }
        ...
        if (prefixString.CompareTo(CUSTOM_HEADER_MODE) == 0)
        {
            ...
            Console.WriteLine("Custom Config File Header length: " + customHeaderLength);
            try
            {
                Console.WriteLine("Removing custom header from config file data. This SEB version cannot process this header type and will ignore it.");

                byte[] customHeaderData = GetPrefixDataFromData(ref sebData, customHeaderLength);

                Console.WriteLine("Custom header data: " + customHeaderData);
            }
            catch (Exception ex)
            {
                Console.WriteLine("Error while removing custom header from config file data", null, ex, ex.Message);
            }
        }
        ...
        if (prefixString.CompareTo(PASSWORD_MODE) == 0)
        {
            ...
            do
            {
                i--;
                // Prompt for password
                Console.Write("Prompt for password: ");
            ...
            if (sebDataDecrypted == null)
            {
                ...
                Console.WriteLine("Wrong password entered in 5th try: stop reading .seb file");
                return null;
            }
            ...
            else
            {
                ...
                if (prefixString.CompareTo(PLAIN_DATA_MODE) != 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Console.WriteLine("No valid prefix and no unencrypted file with valid header: cancel reading .seb file");
                        }
                        return null;
                    }
                }
    ...
    private static DictObj DecryptDataWithPasswordForConfiguringClient(byte[] sebData, bool forEditing, ref string sebFilePassword, ref bool passwordIsHash)
    {
        ...
        catch (Exception readPlistException)
        {
            ...
            Console.WriteLine("Error when deserializing the decrypted configuration data: " + readPlistException.Message);
    ...
    private static byte[] DecryptDataWithPublicKeyHashPrefix(byte[] sebData, bool usingSymmetricKey, bool forEditing, ref X509Certificate2 sebFileCertificateRef)
    {
        ...
        {
            // No certificate found for this public key hash
            Console.WriteLine("No certificate found for this public key hash: " + publicKeyHash);
            return null;
        }
    ...
    public static byte[] EncryptSEBSettingsWithCredentials(string settingsPassword, bool passwordIsHash, X509Certificate2 certificateRef, bool useAsymmetricOnlyEncryption, SEBSettings.sebConfigPurposes configPurpose, bool forEditing)
    {
        ...
        else
        {
            ...
            if (String.IsNullOrEmpty(settingsPassword) && certificateRef == null)
            {
                ...
                // Console.WriteLine("No encryption chosen: save unencrypted? (y/n)");
