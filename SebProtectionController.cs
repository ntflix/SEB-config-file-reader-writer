...
public class SEBProtectionController
{
    ...
    public static void StoreCertificateIntoStore(byte[] certificateData)
    {
        ...
        {
            Console.WriteLine("The X509 store in Windows Certificate Store could not be opened: ", null, storeOpenException, storeOpenException.Message);
        }
        ...
        {
            Console.WriteLine("The X509 Trusted People store in Windows Certificate Store for Local Machine could not be opened: ", null, storeOpenException, storeOpenException.Message);
            return;
        }
    ...
    public static void StoreCertificateIntoPassedStore(byte[] certificateData, X509Store store)
    {
        ...
        {
            Console.WriteLine("The identity data could not be imported into the X509 certificate store.", null, certImportException, certImportException.Message);
            store.Close();
            return;
        }
        ...
        {
            Console.WriteLine("The identity could not be added to the Windows Certificate Store", null, certAddingException, certAddingException.Message);
            store.Close();
            return;
        }
        Console.WriteLine("The identity was successfully added to the Windows Certificate Store");
        store.Close();
    }
    ...
    public static byte[] DecryptDataWithCertificate(byte[] encryptedData, X509Certificate2 sebCertificate)
    {
        ...
        {
            Console.WriteLine("Decrypting SEB config data encrypted with an identity failed with cryptographic exception:", null, cex, cex.Message);
            return null;
        }
        catch (Exception ex)
        {
            Console.WriteLine("Decrypting SEB config data encrypted with an identity failed with exception:", null, ex, ex.Message);
            return null;
        }
    }
    ...
    public static string ComputeBrowserExamKey()
    {
        ...
        {
            configurationKey = "";
            Console.WriteLine("The current configuration does not contain a value for the configuration key!");
        }

        if (salt == default || salt.Length == 0)
        {
            salt = new byte[0];
            Console.WriteLine("The current configuration does not contain a salt value for the browser exam key!");
        }
        ...
        {
            Console.WriteLine("Successfully calculated BEK using integrity module.");
        }
        else
        {
            Console.WriteLine("Failed to calculate BEK using integrity module! Falling back to simplified calculation...");
    ...
    private static bool TryCalculateBrowserExamKey(string configurationKey, string salt, out string browserExamKey)
    {
        ...
        {
            Console.WriteLine("Integrity module is not available!");
        }
        catch (Exception e)
        {
            Console.WriteLine("Unexpected error while attempting to calculate browser exam key!", default, e);
        }
